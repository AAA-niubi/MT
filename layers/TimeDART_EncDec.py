...
class Diffusion(nn.Module):
    ...
    def noise(self, x, t):
        ...
        # 计算输入信号的标准差，用于归一化噪声
        x_std = torch.std(x, dim=(1, 2), keepdim=True) + 1e-8  # [batch_size * num_features, 1, 1]
        
        # 将噪声缩放到与信号相同的量纲
        noise_normalized = noise * x_std
        
        gamma_t = self.gamma[t].unsqueeze(-1)  # [batch_size * num_features, seq_len, 1]
        # x_t = sqrt(gamma_t) * x + sqrt(1 - gamma_t) * noise_normalized
        noisy_x = torch.sqrt(gamma_t) * x + torch.sqrt(1 - gamma_t) * noise_normalized
        return noisy_x, noise_normalized
